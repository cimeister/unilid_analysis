...
for their script and which attract misclassified samples for the same reason
(the identification criteria are stated in \cref{app:protocol}). Both patterns
...
$\vocab$ not observed in $\corpus^\lang$ is replaced with one shared
constant, $\log\unigramdistlang[\token] \defeq c$ for every $\token$ unseen
in $\corpus^\lang$. Without this correction, that value is a byproduct of
the training-time probability floor of $10^{-12}$ and renormalization, and
it varies with $|\corpus^\lang|$; since \cref{eq:lang-id} compares scores
across languages, the variation acts as a language-dependent offset on every
unseen token.
The second correction discounts unreliable wins by the two identified
groups. Let $s_\lang(\str) = \log
...
unchanged. Each threshold $\tau_\lang$ is a percentile of the margins
$\lang$ attains on its own training samples, so a language's threshold
bounds its own recall loss by construction.
This methodology avoids fitting to the evaluation data: each $\tau_\lang$
comes from language $\lang$'s own training samples, the remaining constants
were selected on a retired validation sample and on a development subset of
the GlotLID-C pool, and the held-out subset behind the confirmation
intervals we report, as well as UDHR, FLORES-200, and CommonLID, entered no
constant selection (with one exception, the coarse 100{,}000-sample bar,
fixed before the development split existed; \cref{app:protocol} lists which
data selected each constant). Apart from the high-entropy-group
...
West Flemish. The percentile defining $\tau_\lang$ is
$q_\lang = 5\,(1 - N_\lang/18{,}000)$ for a language with $N_\lang$ training
samples, and a fixed 5th percentile for the high-entropy group; margins are
computed under the distributions with the unseen-token constant applied. A
...
re-examination, held-out macro F1 rises from 0.912 to 0.930.

\paragraph{Provenance of the constants.} The unseen-token constant
$c = -21$ was chosen from a sweep over $\{-17, -19, -21, -23\}$ on a
held-out validation subset of GlotLID-C; the proximity bound $21$ from a
grid over $0.5$ to $100$ in steps of $1$ (the optimum is a plateau spanning
roughly $15$ to $35$) on a development subset of the test pool; the
$100{,}000$-sample bar from an analysis of where reassignments send their
false positives. The misclassified-validation-sample count in the
high-entropy-group criterion is computed on the retired validation sample. All
...
(27{,}002{,}441 lines). The unseen-token constant was selected on a retired
