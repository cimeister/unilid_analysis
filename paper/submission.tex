...
Simple post-training calibration steps (calibrated \unilid, \cref{sec:calibration}) raise macro F1 on the 1{,}940-language GlotLID-C test set from .929 to .957, outperforming every system with full GlotLID-C label coverage.
...
The number of edges $E$ in a lattice is $N\cdot b$, where $b$ is the lattice's branching factor, \ie, how many tokens in the vocabulary start at a given string position and are part of a valid segmentation (typically 1-5), so inference takes $\mathcal{O}(|\languages|\cdot N\cdot b + N \cdot\maxtoklength)$.
...
\defn{Calibrated \unilid} adds two inference-time
corrections.
First, it replaces the log-probability that each language assigns to tokens in $\vocab$ that are not seen in $\corpus^\lang$ with one shared constant across all languages: $\log\unigramdistlang[\token] \defeq c$ for every $\token$ unseen in $\corpus^\lang$.
Without intervention, this value ends up being a
byproduct of the training-time smoothing floor and renormalization, varying largely as a function of $|\corpus^\lang|$.
%Since \cref{eq:lang-id} compares scores across languages, these values act as language-dependent offsets on every unseen token.
Second, it re-examines low-margin predictions into languages with fewer than 18{,}000 training samples (and four large-corpus languages with unusually flat token distributions), i.e., predictions into such languages where the top-scoring and runner-up languages' scores are very close.
%This is a particularly effective step for low-resource languages, where noisy estimates cause samples to be incorrectly assigned as that language.
Concretely, let $s_\lang(\str) = \log \punigramlang(\tokfunc_{\unigramdistlang}(\str))$ be the score assigned by language $\lang$ and
$\hat\lang = \argmax_\lang s_\lang(\str)$ be our predicted language.
If $\hat\lang$ is in the re-examined set and $s_{\hat\lang}(\str)$ exceeds the runner-up score by less than a threshold $\tau_{\hat\lang}$, the prediction moves to the highest-ranked of the candidates ranked 2 to 5 that has at least 100{,}000 training samples and a score within 21 natural-log units of the top candidate; if no candidate qualifies, the prediction is unchanged.
Each threshold $\tau_\lang$ is a percentile of the margins $\lang$ attains on its own training samples; apart from the flat-language diagnostic, both corrections depend only on shared constants and on each language's own data, preserving \unilid's ability to add languages incrementally.
The constants are selected on data disjoint from the subset on which we report their effect; \cref{app:protocol} gives the provenance of each.
...
We set each margin threshold $\tau_\lang$ at the $q_\lang = 5\,(1 - N_\lang/18{,}000)$ percentile of language $\lang$'s own training margins for the languages below 18{,}000 samples, and at a fixed 5th percentile for the four flat languages.
...
from 0.929 to 0.957, above every system in \cref{tab:lid_main} with full
GlotLID-C label coverage, while
lowering macro FPR from 2.03e-5 to 1.77e-5; on a held-out subset outside
every calibration-constant selection except the 100{,}000-sample bar
(\cref{app:protocol}), the paired bootstrap interval of the
improvement over \unilid is $+0.038$ with 95\% interval $[+0.033, +0.043]$
(\cref{tab:calibrated_heldout}). The gain sits where \cref{tab:calibrated_views}
...
\cref{app:protocol} reports the balanced-draw measurement of the same trade.
On out-of-domain web-domain data (CommonLID, \cref{tab:commonlid}),
calibration raises macrolanguage-aware accuracy from 0.845 to 0.860. The
...
$100{,}000$-sample bar, fixed from a false-positive profile measured on the
whole post-draw pool before this split existed.
...
shared threshold, with the replacement-candidate bar also lowered to
