...
Simple post-training calibration steps raise macro F1 on the 1{,}940-language GlotLID-C test set from .929 to .957, outperforming every system we compare against. 
...
Calibrated \unilid adds two inference-time
corrections. 
First, it replaces the unigram probability that each language assigns to tokens in $\vocab$ that are not seen in $\corpus^\lang$ with one shared constant across all languages: $\log\unigramdistlang[\token] \defeq c$ for every $\token$ unseen in $\corpus^\lang$.  
Without intervention, this probability ends up being a
byproduct of the training-time smoothing floor and renormalization, varying largely as a function of $|\corpus^\lang|$. 
%Since \cref{eq:lang-id} compares scores across languages, these values act as language-dependent offsets onevery unseen token. 
Second, it re-examines low-margin predictions, i.e., predictions where the margin between the scores assigned by the top-scoring and runner-up languages are very close. 
...
We learn a per-language threshold $\tau_\lang$ such that if language $\hat{\lang}$ is the highest scoring language and the score it assigns is less than $\tau_{\hat\lang}$ apart from the runner-up, then we re-assign it to the next highest-scoring language whose $\tau_\lang$ is larger than its margin from  $\hat{\lang}$. %and that languages training da when the margin between 
Both corrections depend only on shared constants and on each language's own data, preserving \unilid's ability to add languages incrementally.
We derive these constants using a validation set, which we discuss further in \cref{app:protocol} .
%gives the full specification and the provenance of everyconstant; \cref{sec:exp_setup} lists the values used, and \cref{sec:results}reports the measured effect.
...
We set each margin threshold $\tau_\lang$ at the $q_\lang = 5\,(1 - N_\lang/18{,}000)$ percentile of language $\lang$'s own training margins. %; replacement candidates need at least 100{,}000 training samples and a score within 21 natural-log units of the top candidate. 
