...
Thus, for a fairer comparison, we provide systems' results computed over only the subset of languages covered by \cld (right side of \cref{tab:lid_main}). \corrrev{Each system in those columns is fitted to the subset's languages; \cref{tab:cld3_refit} reports what the full 1{,}940-language \unilid and its calibrated form score on the same slices.}
...
calibration of \cref{tab:lid_main}'s subset rows moves each benchmark in the same
direction as it moves the full system on the same slices: up on GlotLID-C and
