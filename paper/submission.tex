...
Interpretable post-training calibration steps---a shared floor probability across languages for unseen-tokens and calibration of prediction margins for under-resourced---languages raise macro F1 on the 1{,}940-language GlotLID-C test set from .929 to .957, above every system we compare against, while lowering the false positive rate from 2.0e-5 to 1.8e-5.
...
Taken together, these results suggest that treating segmentation as language-specific, rather than as a fixed preprocessing step, enables more effective LID systems.
...
The estimates $\unigramdistlang$ leave one quantity uncalibrated across
languages: the probability of tokens unseen in $\corpus^\lang$, which is a
byproduct of the training-time smoothing floor and renormalization and
therefore varies with $|\corpus^\lang|$. Since \cref{eq:lang-id} compares
scores across languages, these values act as language-dependent offsets on
every unseen token. \defn{Calibrated \unilid} adds two inference-time
corrections. First, it replaces the unseen-token values with one shared
constant: $\log\unigramdistlang[\token] \defeq c$ for every $\token$ unseen in
$\corpus^\lang$, with a single $c$ for all $\lang$. Second, it re-examines
low-margin predictions into under-resourced languages: writing
$s_\lang(\str) = \log \punigramlang(\tokfunc_{\unigramdistlang}(\str))$ and
$\hat\lang = \argmax_\lang s_\lang(\str)$, if $\hat\lang$ has a small training
corpus and $s_{\hat\lang}(\str) - \max_{\lang \neq \hat\lang} s_\lang(\str) <
\tau_{\hat\lang}$, the label moves to the highest-scoring alternative
satisfying a training-resource condition and a score-proximity condition. Each
threshold $\tau_\lang$ is a percentile of the margins $\lang$ attains on its
own training samples, so both corrections depend only on shared constants and
on each language's own data, preserving the incremental-addition property.
\Cref{app:protocol} gives the full specification and the provenance of every
constant; \cref{sec:exp_setup} lists the values used, and \cref{sec:results}
reports the measured effect.
...
To avoid zero-probability segmentations when a token's expected counts are estimated to be 0 under a given language (most common in the few-sample regime), we floor token probabilities at $10^{-12}$ during training. For calibrated \unilid (\cref{sec:calibration}) we use the shared unseen-token constant $c = -21$ (natural log), re-examine predictions into languages with fewer than 18{,}000 training samples (and four large-corpus languages with unusually flat token distributions), and set each margin threshold $\tau_\lang$ at the $q_\lang = 5\,(1 - N_\lang/18{,}000)$ percentile of language $\lang$'s own training margins; replacement candidates need at least 100{,}000 training samples and a score within 21 natural-log units of the top candidate. The full protocol, remaining constants, and their provenance are in \cref{app:protocol}.
...
from 0.929 to 0.957, above every system in \cref{tab:lid_main}, while
lowering macro FPR from 2.03e-5 to 1.77e-5; on a held-out subset that no
calibration constant was selected on, the paired bootstrap interval of the
improvement over \unilid is $+0.038$ with 95\% interval $[+0.033, +0.043]$
(\cref{tab:calibrated_heldout}). The gain sits where \cref{tab:resource-tier}
reports \unilid's weakness: for languages with under 1{,}000 training
samples, mean per-language F1 rises from 0.332 to 0.732 (held-out subset, all
false positives counted). The gains are specific to natural-distribution test
data, where false-positive volume into under-resourced languages matters: on
the parallel, equal-support UDHR set calibration lowers macro F1 (0.859 to
0.838) while FLORES-200 changes little (0.932 to 0.933), and
\cref{app:protocol} reports both views of this trade. On out-of-domain
web-domain data (CommonLID, \cref{tab:commonlid}), calibration raises
macrolanguage-aware accuracy from 0.845 to 0.860. On benchmarks restricted
to larger languages the added mechanisms rarely fire.
...
language has fewer than 18{,}000 training samples, or is one of four
large-corpus languages whose token distributions are unusually flat for their
script (Scots, Banjar, Aragonese, and West Flemish), and (ii) the top score
exceeds the runner-up score by less than the predicted language's threshold
$\tau_\lang$. The percentile defining $\tau_\lang$ is
$q_\lang = 5\,(1 - N_\lang/18{,}000)$ for a language with $N_\lang$ training
samples, and a fixed 5th percentile for the four flat languages; margins are
computed under the calibrated (shared-floor) distributions. A re-examined
prediction is reassigned to the highest-ranked of the candidates ranked 2 to
5 that has at least 100{,}000 training samples and a score within 21
natural-log units of the top candidate; if no candidate qualifies, the
prediction is unchanged. Under the shared floor alone, held-out macro F1
rises from 0.912 to 0.930 (the floor's isolated contribution).
...
which was excluded from every one of these selections.

\paragraph{Development protocol.} The constants were selected on data disjoint from
...
the improvement acts on, so the gain does not appear there.
\Cref{tab:commonlid} reports an out-of-domain web-domain evaluation
(CommonLID): calibration raises macrolanguage-aware accuracy from 0.845 to
0.860 while tag-level macro F1 moves from 0.723 to 0.715, and the caption
states the mechanism. Both measurements bound where the method's gains should
be expected: on natural-distribution traffic over a label set that includes
