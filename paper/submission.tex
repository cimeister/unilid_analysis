...
Simple post-training calibration steps (calibrated \unilid, \cref{sec:calibration}) raise macro F1 on the 1{,}940-language GlotLID-C test set from \corrrev{0.933} to \corrrev{0.956}, \camrev{above every system we evaluate on the full GlotLID-C label set (\cref{tab:lid_main})}.
...
\corrrev{arise from} under-fit distribution estimates. \defn{Calibrated \unilid}
...
for every $\token$ unseen in $\corpus^\lang$. For 1{,}655 of the 1{,}940
...
of them are under-resourced, calibration lowers macro F1 from 0.856 to 0.842.
...
calibration of \cref{tab:lid_main}'s subset rows changes each benchmark's macro F1 in the same
direction as it changes that of the full system on the same slices: up on GlotLID-C and
FLORES-200, down on UDHR. Most of that GlotLID-C gain is one language: Corsican
...
alone. The calibrated row's \cld-subset cells apply the procedure of
...
which the procedure defines as functions of the model being calibrated. The
...
\corrrev{0.862} while tag-level macro F1 \corrrev{falls} from \corrrev{0.722} to \corrrev{0.717}\camrev{: the
...
\paragraph{Alternatives measured during development.} (i) Recalibrating each language's unseen-token
mass from its own Good-Turing estimate \corrrev{(the estimate that assigns the
