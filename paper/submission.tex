...
$\tau_\lang$, its own unseen-token treatment (two languages whose trained
unseen-token values already lie below $c = -21$ are left unchanged), and
...
effect: calibration raises the variant's macro F1 from 0.913 to 0.954 on
the full pool, and the paired bootstrap interval of the improvement on the
held-out subset is $+0.050$ with 95\% interval $[+0.044, +0.057]$. The gain
is larger than for the dedicated-tokenizer model ($+0.041$ against $+0.028$
