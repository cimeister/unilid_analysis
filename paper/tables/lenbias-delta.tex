...
\toprule
\textbf{Length (chars)} & \corrrev{\textbf{\# Errors}} & \textbf{Mean $\Delta$} & \textbf{Median $\Delta$} & \textbf{\% fewer} & \textbf{\% same} & \textbf{\% more} \\
\midrule
...
\caption{%
  ...
  $9{,}906$ misclassifications in the diagnostic half of the $500{,}000$-line draw
  ($250{,}000$ lines; \cref{app:protocol}), the same subset as \cref{tab:lenbias-norm}; the error-count column is
  ...
  fewer-token languages, \corrrev{produced by the $25\%$ of misclassifications in which the predicted language uses fewer tokens}. \emph{\% fewer/same/more} give the
