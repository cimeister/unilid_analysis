...
Pre-registration: EXPERIMENTS_PLAN.md, "Plan: per-language combined method", 2026-07-29 amendments. Confirmation instrument for configurations whose treatment came to attention through the derivation part of the seed-301 split; analysis/combined_evidence.py derives and records that split, this script only reads it. Requested configs: gate_flat4_tau5. Fixed comparator set: baseline, floor21, gt_min, gt_margin_adaptive.
...
| gate_flat4_tau5 | 2b2c3d898d88b596 | 91,254,686 | 2026-08-06 00:35:01 |
...
| group | n langs | gate_flat4_tau5 | baseline | floor21 | gt_min | gt_margin_adaptive |
|---|---|---|---|---|---|---|
| all 1,940 | 1940 | 0.9486 | 0.9117 | 0.9300 | 0.9111 | 0.9329 |
| tail (N<1k) | 96 | 0.7306 | 0.3319 | 0.6161 | 0.2883 | 0.4523 |
| lowmid (1k-18k) | 984 | 0.9599 | 0.9267 | 0.9352 | 0.9294 | 0.9568 |
| head (N>=18k) | 860 | 0.9600 | 0.9593 | 0.9590 | 0.9596 | 0.9592 |
| flat_magnet | 118 | 0.6545 | 0.2845 | 0.5197 | 0.2476 | 0.4128 |
| twin | 77 | 0.8906 | 0.8858 | 0.8850 | 0.8866 | 0.8909 |
...
- Clause (B): overall global mean F1 gain +0.0369 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.3986, flat_magnet global mean F1 gain +0.3699 (each must be >= -PREC_TOL = -0.0000) (PASS).
...
|---|---|---|---|---|---|
| gate_flat4_tau5 | 0.9737 | 0.8744 | 0.8782 | 0.9407 | 0.9809 |
...
- **gate_flat4_tau5: two-sided ELIGIBLE**
...
|---|---|---|---|
| gate_flat4_tau5 | gt_margin_adaptive | +0.0157 | [+0.0118, +0.0198] |
| gate_flat4_tau5 | floor21_gate | +0.0006 | [+0.0001, +0.0013] |
