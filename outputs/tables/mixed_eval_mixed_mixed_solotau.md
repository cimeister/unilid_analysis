...
Pre-registration: EXPERIMENTS_PLAN.md, "Plan: per-language combined method", 2026-07-29 amendments. Confirmation instrument for configurations whose treatment came to attention through the derivation part of the seed-301 split; analysis/combined_evidence.py derives and records that split, this script only reads it. Requested configs: mixed, mixed_solotau. Fixed comparator set: baseline, floor21, gt_min, gt_margin_adaptive.
...
| mixed | 427207b6c87982f4 | 91,254,686 | 2026-07-30 16:26:10 |
| mixed_solotau | 601962e982a1adb0 | 91,254,686 | 2026-07-30 16:31:38 |
...
| group | n langs | mixed | mixed_solotau | baseline | floor21 | gt_min | gt_margin_adaptive |
|---|---|---|---|---|---|---|---|
| all 1,940 | 1940 | 0.9482 | 0.9479 | 0.9117 | 0.9300 | 0.9111 | 0.9329 |
| tail (N<1k) | 96 | 0.7323 | 0.7316 | 0.3319 | 0.6161 | 0.2883 | 0.4523 |
| lowmid (1k-18k) | 984 | 0.9606 | 0.9602 | 0.9267 | 0.9352 | 0.9294 | 0.9568 |
| head (N>=18k) | 860 | 0.9580 | 0.9580 | 0.9593 | 0.9590 | 0.9596 | 0.9592 |
| flat_magnet | 118 | 0.6464 | 0.6460 | 0.2845 | 0.5197 | 0.2476 | 0.4128 |
| twin | 77 | 0.8910 | 0.8910 | 0.8858 | 0.8850 | 0.8866 | 0.8909 |
...
- Clause (B): overall global mean F1 gain +0.0364 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.4004, flat_magnet global mean F1 gain +0.3619 (each must be >= -PREC_TOL = -0.0000) (PASS).
- Clause (C): 0 supported collapses, verdict **clean**.
### mixed_solotau

- Clause (B): overall global mean F1 gain +0.0362 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.3996, flat_magnet global mean F1 gain +0.3614 (each must be >= -PREC_TOL = -0.0000) (PASS).
- Clause (C): 0 supported collapses, verdict **clean**.
...
|---|---|---|---|---|---|
| mixed | 0.9740 | 0.8713 | 0.8758 | 0.9406 | 0.9813 |
| mixed_solotau | 0.9734 | 0.8703 | 0.8750 | 0.9407 | 0.9813 |
...
- **mixed: two-sided REJECTED** (val tail drops 0.0457 > 0.03; val magnets drops 0.0416 > 0.03)
  uniform-prior track (passes_uniform): FAIL
- **mixed_solotau: two-sided REJECTED** (val tail drops 0.0467 > 0.03; val magnets drops 0.0424 > 0.03)
  uniform-prior track (passes_uniform): FAIL
...
|---|---|---|---|
| mixed | gt_margin_adaptive | +0.0152 | [+0.0114, +0.0193] |
| mixed | floor21_gate | +0.0002 | [-0.0003, +0.0006] |
| mixed_solotau | gt_margin_adaptive | +0.0150 | [+0.0111, +0.0190] |
| mixed_solotau | floor21_gate | -0.0001 | [-0.0006, +0.0003] |
