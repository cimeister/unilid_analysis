...
Pre-registration: EXPERIMENTS_PLAN.md, "Plan: per-language combined method", 2026-07-29 amendments. Confirmation instrument for configurations whose treatment came to attention through the derivation part of the seed-301 split; analysis/combined_evidence.py derives and records that split, this script only reads it. Requested configs: floor21_gate, unmod_gate. Fixed comparator set: baseline, floor21, gt_min, gt_margin_adaptive.
...
| group | n langs | floor21_gate | unmod_gate | baseline | floor21 | gt_min | gt_margin_adaptive |
|---|---|---|---|---|---|---|---|
| all 1,940 | 1940 | 0.9480 | 0.9328 | 0.9117 | 0.9300 | 0.9111 | 0.9329 |
| tail (N<1k) | 96 | 0.7306 | 0.4592 | 0.3319 | 0.6161 | 0.2883 | 0.4523 |
| lowmid (1k-18k) | 984 | 0.9599 | 0.9561 | 0.9267 | 0.9352 | 0.9294 | 0.9568 |
| head (N>=18k) | 860 | 0.9586 | 0.9590 | 0.9593 | 0.9590 | 0.9596 | 0.9592 |
| flat_magnet | 118 | 0.6435 | 0.4246 | 0.2845 | 0.5197 | 0.2476 | 0.4128 |
| twin | 77 | 0.8908 | 0.8919 | 0.8858 | 0.8850 | 0.8866 | 0.8909 |
...
|---|---|---|
| floor21_gate | 1,912 | 117,921 |
| unmod_gate | 5,648 | 132,019 |
...
- Clause (B): overall global mean F1 gain +0.0363 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.3986, flat_magnet global mean F1 gain +0.3589 (each must be >= -PREC_TOL = -0.0000) (PASS).
- Clause (C): 0 supported collapses, verdict **clean**.
### unmod_gate

- Clause (B): overall global mean F1 gain +0.0211 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.1273, flat_magnet global mean F1 gain +0.1401 (each must be >= -PREC_TOL = -0.0000) (PASS).
- Clause (C): 0 supported collapses, verdict **clean**.
...
|---|---|---|---|---|---|
| floor21_gate | 0.9738 | 0.8744 | 0.8790 | 0.9403 | 0.9810 |
| unmod_gate | 0.9777 | 0.9042 | 0.9039 | 0.9407 | 0.9812 |
...
- **floor21_gate: two-sided REJECTED** (val tail drops 0.0426 > 0.03; val magnets drops 0.0383 > 0.03)
  uniform-prior track (passes_uniform): FAIL
- **unmod_gate: two-sided ELIGIBLE**
  uniform-prior track (passes_uniform): FAIL
...
|---|---|---|---|
| floor21_gate | gt_margin_adaptive | +0.0151 | [+0.0112, +0.0191] |
| unmod_gate | gt_margin_adaptive | -0.0001 | [-0.0021, +0.0018] |
| unmod_gate | floor21_gate | -0.0152 | [-0.0185, -0.0120] |
