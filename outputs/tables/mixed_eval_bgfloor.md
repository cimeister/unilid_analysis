...
Pre-registration: EXPERIMENTS_PLAN.md, "Plan: per-language combined method", 2026-07-29 amendments. Confirmation instrument for configurations whose treatment came to attention through the derivation part of the seed-301 split; analysis/combined_evidence.py derives and records that split, this script only reads it. Requested configs: bgfloor. Fixed comparator set: baseline, floor21, gt_min, gt_margin_adaptive.
...
| bgfloor | af09869930d0d581 | 91,254,686 | 2026-08-06 03:57:37 |
...
| group | n langs | bgfloor | baseline | floor21 | gt_min | gt_margin_adaptive |
|---|---|---|---|---|---|---|
| all 1,940 | 1940 | 0.9304 | 0.9117 | 0.9300 | 0.9111 | 0.9329 |
| tail (N<1k) | 96 | 0.6196 | 0.3319 | 0.6161 | 0.2883 | 0.4523 |
| lowmid (1k-18k) | 984 | 0.9354 | 0.9267 | 0.9352 | 0.9294 | 0.9568 |
| head (N>=18k) | 860 | 0.9593 | 0.9593 | 0.9590 | 0.9596 | 0.9592 |
| flat_magnet | 118 | 0.5229 | 0.2845 | 0.5197 | 0.2476 | 0.4128 |
| twin | 77 | 0.8861 | 0.8858 | 0.8850 | 0.8866 | 0.8909 |
...
- Clause (B): overall global mean F1 gain +0.0187 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.2877, flat_magnet global mean F1 gain +0.2384 (each must be >= -PREC_TOL = -0.0000) (PASS).
...
|---|---|---|---|---|---|
| bgfloor | 0.9802 | 0.8936 | 0.8974 | 0.9410 | 0.9815 |
...
|---|---|---|---|
| bgfloor | gt_margin_adaptive | -0.0025 | [-0.0068, +0.0017] |
| bgfloor | floor21_gate | -0.0176 | [-0.0208, -0.0145] |
