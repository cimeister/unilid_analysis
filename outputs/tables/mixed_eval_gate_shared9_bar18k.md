...
Pre-registration: EXPERIMENTS_PLAN.md, "Plan: per-language combined method", 2026-07-29 amendments. Confirmation instrument for configurations whose treatment came to attention through the derivation part of the seed-301 split; analysis/combined_evidence.py derives and records that split, this script only reads it. Requested configs: gate_shared9_bar18k. Fixed comparator set: baseline, floor21, gt_min, gt_margin_adaptive.
...
| gate_shared9_bar18k | 773b9e09ddaccc92 | 91,254,686 | 2026-08-06 00:05:34 |
...
| group | n langs | gate_shared9_bar18k | baseline | floor21 | gt_min | gt_margin_adaptive |
|---|---|---|---|---|---|---|
| all 1,940 | 1940 | 0.9534 | 0.9117 | 0.9300 | 0.9111 | 0.9329 |
| tail (N<1k) | 96 | 0.7579 | 0.3319 | 0.6161 | 0.2883 | 0.4523 |
| lowmid (1k-18k) | 984 | 0.9714 | 0.9267 | 0.9352 | 0.9294 | 0.9568 |
| head (N>=18k) | 860 | 0.9547 | 0.9593 | 0.9590 | 0.9596 | 0.9592 |
| flat_magnet | 118 | 0.6869 | 0.2845 | 0.5197 | 0.2476 | 0.4128 |
| twin | 77 | 0.8766 | 0.8858 | 0.8850 | 0.8866 | 0.8909 |
...
|---|---|---|
| gate_shared9_bar18k | 810 | 81,517 |
...
- Clause (B): overall global mean F1 gain +0.0417 (must be >= -GUARD_TOL = -0.0100), tail global mean F1 gain +0.4260, flat_magnet global mean F1 gain +0.4023 (each must be >= -PREC_TOL = -0.0000) (PASS).
- Clause (C): 9 supported collapse(s), verdict **class-level fail**: bpr_Latn (F1 0.8474 vs baseline 0.9490, support 925); bps_Latn (F1 0.8375 vs baseline 0.9598, support 1132); cya_Latn (F1 0.7514 vs baseline 0.8739, support 893); llb_Latn (F1 0.5716 vs baseline 0.9244, support 2523); mrj_Cyrl (F1 0.7235 vs baseline 0.8403, support 2947); nhe_Latn (F1 0.8233 vs baseline 0.9358, support 1366); thv_Latn (F1 0.0000 vs baseline 0.1263, support 15); twx_Latn (F1 0.5817 vs baseline 0.7056, support 2634); zgh_Tfng (F1 0.6148 vs baseline 0.9292, support 184)
- Informational, below the support floor, exempt from clause (C): max_Latn (F1 0.0000 vs baseline 0.1034, support 7); tzl_Latn (F1 0.0000 vs baseline 0.1935, support 3)
...
|---|---|---|---|---|---|
| gate_shared9_bar18k | 0.9684 | 0.8206 | 0.8057 | 0.9196 | 0.9817 |
...
- **gate_shared9_bar18k: two-sided REJECTED** (val overall drops more than 0.01; val tail drops 0.0964 > 0.03 (TAIL_RECALL_TOL); val magnets drops 0.1117 > 0.03 (TAIL_RECALL_TOL); val twins drops 0.0209 > 0.01 (GUARD_TOL); 9 language(s) with support >= 10 lose more than 0.1 global F1 (worst 0.3529); more than 2 outliers is a class-level pattern)
...
|---|---|---|---|
| gate_shared9_bar18k | gt_margin_adaptive | +0.0205 | [+0.0161, +0.0253] |
| gate_shared9_bar18k | floor21_gate | +0.0054 | [+0.0031, +0.0078] |
