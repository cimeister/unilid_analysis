"""Evaluate .unilid models on UDHR dataset (CSV format)."""
import argparse
import json
import time
...
def evaluate(model_path, csv_path, out_dir=None, lang_only=False, only_model_langs=True, forward=False):
    ...
    if lang_only:
        ...
        df["label"] = df["full_label"]
    df["text"] = df["sentence"].astype(str)
    print(f"Loaded {len(df)} samples from UDHR")
    ...
    if only_model_langs:
        df_filtered = df[df["label"].isin(model_labels_check)]
        print(f"Filtered to {len(df_filtered)} samples in {df_filtered['label'].nunique()} model languages (from {len(df)} total)")
        df = df_filtered
    ...
    # Compute metrics only on gold languages (languages present in the dataset)
    ...
    if out_dir:
        ...
        with open(out_dir / f"{model_name}_udhr_metrics.json", "w") as f:
            json.dump(summary, f, indent=2)

        with open(out_dir / f"{model_name}_udhr_per_language.json", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv", required=True, help="UDHR CSV path")
    ...
    evaluate(args.model, args.csv, args.out_dir, lang_only=args.lang_only, only_model_langs=not args.all_langs, forward=args.forward)
