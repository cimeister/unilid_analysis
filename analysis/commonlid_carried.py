...
Gate: the recomputed baseline macro-aware accuracy must equal the recorded 0.8452
(Exp 12) at 4 decimals, else abort.

Output: outputs/tables/commonlid_carried.md.
"""
...
def run(out_md: str = OUT_MD) -> str:
    ...
    if n_mod != n_lang:
        raise RuntimeError(f"floor {FLOOR_TARGET} modified {n_mod} rows, expected "
                           f"all {n_lang}")
    scr = os.path.dirname(os.path.join(
        "/capstor/scratch/cscs/cmeister747/unilid_analysis/full_test_eval", "x"))
