#!/bin/bash
#SBATCH --job-name=unilid-deepseek-fulltest
...
#SBATCH --output=/capstor/scratch/cscs/cmeister747/unilid_analysis/logs/deepseek_fulltest_%j.out
#SBATCH --error=/capstor/scratch/cscs/cmeister747/unilid_analysis/logs/deepseek_fulltest_%j.err
set -euo pipefail
#
# Full-pool baseline scoring of the RETRAINED DeepSeek3.2 variant, for its
# tab:lid_main row. Chained with --dependency=afterok on the retrain (job 3112879)
...
MODEL="/capstor/scratch/cscs/cmeister747/unilid_analysis/glotlid_deepseek_v32_fp64.unilid"
if [ ! -f "${MODEL}" ]; then
  echo "FATAL: ${MODEL} missing; the retrain did not pack a model" >&2
  exit 1
fi
echo "=== DeepSeek3.2 retrained full-pool baseline at $(date) on $(hostname) ==="
echo "commit: $(git rev-parse HEAD)"
${PYTHON} -u -m analysis.full_test_eval \
    --model "${MODEL}" \
    --scratch-dir "/capstor/scratch/cscs/cmeister747/unilid_analysis/full_test_eval_deepseek_fp64" \
    --configs baseline \
    --out-dir outputs_deepseek_fp64
